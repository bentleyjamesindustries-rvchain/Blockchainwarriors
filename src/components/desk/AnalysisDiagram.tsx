...
export function AnalysisDiagram({
  ...
  const phase = dead
    ? "Price already took out the stop. This idea is done."
    : chasing
      ? "Price already ran. Wait for a pullback — don't chase."
      : "Last push and dip mapped. Looking for the dip to hold.";
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 md:p-5">
      ...
      <p className="mt-1 text-sm font-medium tracking-tight">
        {long ? "Long" : "Short"} · stop {formatPrice(lock.invalidation, symbol)}
      </p>
      <svg viewBox="0 0 300 180" className="mt-3 h-44 w-full" role="img" aria-label="Price path with current price">
        ...
        {points.map((p) => (
          <g key={p.label}>
            ...
            <text x={p.x} y={p.y - 8} textAnchor="middle" fill="#8b9088" fontSize="9" fontFamily="IBM Plex Sans, sans-serif">
              {p.label}
            </text>
          </g>
        ))}
        <text x="282" y={y(lock.invalidation) + 3} fill="#c45c4a" fontSize="9" textAnchor="end">
          Stop
        </text>
        <text x="282" y={y(lock.tp1) + 3} fill="#3f9d72" fontSize="9" textAnchor="end">
          Target
        </text>
      </svg>
