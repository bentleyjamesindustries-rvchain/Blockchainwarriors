...
export function PriceTape({
  ...
  if (!lock || !price) {
    return <p className="text-sm text-muted-foreground">Waiting on a live mark.</p>;
  }
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {dead
          ? "On the wrong side of the stop — this count is dead."
          : `${formatPrice(Math.abs(vsInv), symbol)} to the stop · ${formatPrice(Math.abs(vsTp), symbol)} to first target`}
      </p>
        ...
        {levels.map((lv) => (
          <li
            ...
            className={cn(
              ...
              lv.kind === "now" && "bg-secondary",
            )}
          >
            <span
              className={cn(
                "text-sm",
                lv.kind === "now" && "font-medium text-foreground",
                lv.kind === "inv" && "text-short",
                lv.kind === "tp" && "text-long",
                lv.kind === "fib" && "text-muted-foreground",
              )}
            >
              {lv.label}
            </span>
